...
# Generate path from house to house
...
while len(houses_set) != 0:  # not houses_set.isEmpty():  bo mialo byc lepiej; I CRY EVERYTIME
    ...
    if int(fuel_consumption) < 70:
        node_to = houses_set.pop(0)
    else:
        print("Fill Up")
        node_to = _grid.table_nodes[10][19]
        fuel_consumption = 0

    node_to_temp = _grid.table_nodes[node_to.x][node_to.y]
    astar_path += Astar(node_from, node_to_temp, _grid)
    fuel_consumption += node_to.g_cost
    total_fuel_consumption += node_to.g_cost
...
print("\nTotal fuel consumption: {}L".format(int(total_fuel_consumption)))
